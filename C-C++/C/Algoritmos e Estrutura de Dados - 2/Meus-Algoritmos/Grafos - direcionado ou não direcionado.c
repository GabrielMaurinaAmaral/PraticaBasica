...
int verificar_direcao(GrafoLA *grafoLA)
{
    if (grafoLA != NULL)
    {
        GrafoMA *grafoMA = converter_LA_em_MA(grafoLA);
        for (int i = 0; i < grafoMA->V; i++)
            for (int j = 0; j < grafoMA->V; j++)
                if (grafoMA->mat[i][j] != grafoMA->mat[j][i])
                    return 1;
        return 0;
        liberarGMA(grafoMA);
    }
...
int main()
{
    ...
    if (verificar_direcao(grafoLA) == 1)
        printf("direcionado");
    else
